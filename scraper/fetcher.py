...
class HumanBehaviorSimulator:
    ...
    @staticmethod
    def simulate_scroll(page: Page, scroll_times: Optional[int] = None):
        """模拟随机滚动页面"""
...
class PlaywrightFetcher:
    ...
    def start(self):
        ...
        try:
            ...
            if getattr(sys, "frozen", False):
                ...
                logger.info(
                    f"[DEBUG] Frozen mode detected. PLAYWRIGHT_BROWSERS_PATH={browsers_path}"
                )

                # Check if path exists, if not, try root variants
                if not browsers_path or not os.path.exists(browsers_path):
                    logger.warning(
                        f"[WARNING] Browsers path not found: {browsers_path}"
                    )
                ...
                if browsers_path and os.path.isdir(browsers_path):
                    ...
                    patterns = [
                        os.path.join(
                            browsers_path, "chromium-*", "chrome-win", exe_name
                        ),
                    ...
                    if matches:
                        ...
                        logger.warning(
                            f"[WARNING] Browser not found. Tried patterns: {patterns}"
                        )
                        # List what's in the browsers directory
                        try:
                            contents = os.listdir(browsers_path)
                            logger.warning(
                                f"[DEBUG] Browsers directory contents: {contents}"
                            )
                        except Exception as e:
                            logger.warning(
                                f"[DEBUG] Could not list browsers directory: {e}"
                            )
                else:
                    logger.warning(
                        f"[WARNING] Browsers path not found or not a directory: {browsers_path}"
                    )
            ...
            self._request_count = 0
            logger.info(
                f"浏览器已启动 (类型={BROWSER_TYPE}, 无头={BROWSER_HEADLESS}, 人类模拟={SIMULATE_HUMAN_BEHAVIOR})"
            )
    ...
    def get_page(
        self, url: str, wait_for: str = "domcontentloaded", is_detail: bool = True
    ) -> Optional[str]:
        """
        获取页面内容
        ...
        try:
            ...
            # 请求前随机短暂延迟
            if self._request_count > 1 and max(DELAY_MIN, DELAY_MAX) > 0:
            ...
            if wait_for == "networkidle":
                self.page.goto(
                    url, wait_until="networkidle", timeout=BROWSER_NAVIGATION_TIMEOUT
                )
            elif wait_for == "load":
                self.page.goto(
                    url, wait_until="load", timeout=BROWSER_NAVIGATION_TIMEOUT
                )
            else:
                self.page.goto(
                    url,
            ...
            if SIMULATE_HUMAN_BEHAVIOR:
                ...
                # 模拟阅读（滚动 + 延迟）
                if is_detail and max(DETAIL_DELAY_MIN, DETAIL_DELAY_MAX) > 0:
                    self.human_sim.simulate_reading(
                        self.page, DETAIL_DELAY_MIN, DETAIL_DELAY_MAX
                    )
                else:
                    self.human_sim.simulate_scroll(self.page, 1)
                    if max(DELAY_MIN, DELAY_MAX) > 0:
            ...
            logger.debug(
                f"成功获取页面 [{self._request_count}]: {url[:60]}... (长度: {len(html)})"
            )
            return html
