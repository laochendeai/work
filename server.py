...
if getattr(sys, "frozen", False):
    ...
    print(
        f"[DEBUG] PLAYWRIGHT_BROWSERS_PATH: {os.environ.get('PLAYWRIGHT_BROWSERS_PATH')}"
    )
    # --------------------
...
app = FastAPI(title=f"Bidding Scraper UI v{APP_VERSION}", lifespan=lifespan)
...
async def check_license_middleware(request, call_next):
    ...
    if path.startswith("/api/") and current_access.get("locked"):
        return JSONResponse(
            status_code=403, content={"error": "License required", "locked": True}
        )
...
async def verify_auth_key(body: LicenseKey):
    ...
    if license_utils.verify_license(code, body.key):
        ...
        return {"success": True}
    return JSONResponse(
        status_code=400, content={"error": "Invalid License Key", "success": False}
    )
...
async def get_announcements(
    limit: int = 50, offset: int = 0, q: str = "", province: str = ""
):
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
...
async def export_announcements(
    ...
    try:
        ...
        try:
            ...
            return JSONResponse(
                status_code=500, content={"error": "openpyxl not installed"}
            )
        ...
        if export_type == "all":
            ...
            return StreamingResponse(
                ...
                headers={
                    "Content-Disposition": "attachment; filename=announcements_by_province.zip"
                },
            )
            ...
            return StreamingResponse(
                ...
                media_type="application/zip",
                headers={
                    "Content-Disposition": "attachment; filename=announcements_by_day.zip"
                },
            )

        else:
            return JSONResponse(
                status_code=400, content={"error": "Invalid export type"}
            )
...
@app.get("/api/cards")
async def get_cards(
    limit: int = 50, offset: int = 0, q: str = "", role: str = "", province: str = ""
):
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        ...
        bcm_join = (
            "INNER JOIN business_card_mentions bcm ON bcm.business_card_id = bc.id"
            if need_inner_join
            else "LEFT JOIN business_card_mentions bcm ON bcm.business_card_id = bc.id"
        )
...
async def get_card_mentions(card_id: int):
    try:
        ...
        cursor.execute(
            "SELECT company, contact_name FROM business_cards WHERE id = ?", (card_id,)
        )
...
async def save_keywords(req: KeywordsRequest):
    try:
        ...
        data = {"keywords": req.keywords, "filename": req.filename}
